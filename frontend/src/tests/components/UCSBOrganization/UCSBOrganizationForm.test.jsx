...
describe("UCSBOrganizationForm", () => {
    ...
    test("required validations prevent submit (create)", async () => {
        ...
        await user.type(screen.getByTestId(`${testId}-orgTranslationShort`), "Associated Students");
        await user.type(screen.getByTestId(`${testId}-orgTranslation`), "Associated Students UCSB");
        await user.click(screen.getByTestId(`${testId}-submit`));
    ...
    test("orgCode must match pattern (uppercase A–Z/0–9, 2–10 chars); shows pattern message on bad input", async () => {
        ...
        const patternMsg = /Code must be 2(?:–|-)?10 uppercase letters\/numbers \(e\.g\.,?\s*SKY, ZPR\)\.?/i;
